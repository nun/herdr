...
"#,
    ...
    #[test]
    fn last_pane_and_attention_agent_keybinds_are_unset_by_default() {
        let kb = Config::default().keybinds();
        assert!(kb.last_pane.bindings.is_empty());
        assert!(kb.previous_attention_agent.bindings.is_empty());
        assert!(kb.next_attention_agent.bindings.is_empty());
    }
