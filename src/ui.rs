...
pub(crate) use self::{
    ...
    sidebar::{
        ...
        agent_panel_needs_attention, agent_panel_scroll_for_target, agent_panel_scroll_metrics,
...
mod tests {
    ...
    fn keybind_help_shows_unset_for_optional_actions() {
        ...
        assert!(panes
            .iter()
            .any(|(key, label)| key == "prefix+left" && label.as_ref() == "focus pane left"));
        assert!(panes
            .iter()
            .any(|(key, label)| key == "prefix+down" && label.as_ref() == "focus pane down"));
        assert!(panes
            .iter()
            .any(|(key, label)| key == "prefix+up" && label.as_ref() == "focus pane up"));
        assert!(panes
            .iter()
            .any(|(key, label)| key == "prefix+right" && label.as_ref() == "focus pane right"));
    }
