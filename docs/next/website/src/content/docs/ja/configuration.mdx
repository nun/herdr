...
任意で、シェルコマンドを一定間隔で実行し、デスクトップのタブバー右端に tmux 風のステータス帯を描画できます:
...
表示されるのは stdout の先頭行だけです（Unicode は可、v1 では ANSI スタイルは適用されません）。表示専用で、デスクトップのタブバー表示に連動します（`ui.hide_tab_bar_when_single_tab` を含む）。`command` が空または省略されていると無効になります。既定値は[設定リファレンス](/docs/config-reference/)で `ui.tab_status` を検索してください。

デスクトップのタブバーは、ステータス帯のさらに右側にアクティブなスペース名（サイドバーのスペース一覧と同じラベル）も描画します。スペース切替で更新され、タブバー非表示時は一緒に消えます。
