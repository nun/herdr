...
focus_attention_agent = "prefix+a"
```

`focus_attention_agent` jumps to the next agent waiting on you: either a blocked agent that needs input, or a "done" agent that finished while you were elsewhere and you have not looked at yet. It prefers the most recent of these and skips the pane you are already on, so pressing it repeatedly walks through every waiting agent. Focusing a done agent marks it seen, dropping it from the rotation. It is a no-op when nothing else is waiting. Set it to an empty string to disable it. (`focus_done_agent` is still accepted as an alias for this binding.)

`toggle_sidebar` (default `prefix+b`) fully hides or shows the sidebar. When the sidebar is visible, the compact collapsed bar is still available from the sidebar's mouse `«`/`»` controls.
...
`last_tab` toggles to the previously active tab in the current workspace, tmux last-window style. It is unset by default; bind it to something like `prefix+tab` for quick back-and-forth tab switching.
...
`sidebar_hidden` starts Herdr with the sidebar fully hidden. The default is `false`. Use `toggle_sidebar` (default `prefix+b`) to show it again.
...
`confirm_close` controls whether closing a workspace asks for confirmation. `prompt_new_tab_name` controls whether new tabs ask for a label first. `prefix_hint` controls whether the prefix-mode hint bar appears at the bottom while prefix mode is active.
...
`terminal_title` controls the outer terminal window title. With the default `"space"`, Herdr updates the title to the active space name whenever you switch spaces. Set `terminal_title = "off"` to leave the title untouched, for example when your window manager or `herdr terminal title set` manages it instead. An explicit `herdr terminal title set` value stays until the next space switch.
