...
focus_done_agent = "prefix+a"
```

`focus_done_agent` jumps to the most recently completed agent, the one that just finished and you have not looked at yet (status "done"). Pressing it focuses that pane, which marks it seen, so pressing it again walks down to the next-most-recent done agent. It is a no-op when no agents are waiting to be reviewed. Set it to an empty string to disable it.
