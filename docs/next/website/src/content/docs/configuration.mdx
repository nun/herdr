...
`last_pane` switches back to the last focused pane across workspaces and tabs. It is unset by default.

`last_tab` toggles back to the previously visited tab within the active workspace, and `last_workspace` toggles back to the previously visited workspace, both tmux-`last-window`-style. They default to `prefix+l` and `prefix+shift+l`, which is why pane focus/swap moved to `prefix+left/down/up/right` and `prefix+shift+left/down/up/right`.

`pin_workspace` pins or unpins the current workspace. Pinned workspaces are kept at the top of the sidebar space list, separated from the rest by a divider, and stay pinned across restarts. Unpinning moves the workspace back below the remaining pinned ones. It defaults to `prefix+shift+f`.
...
### Tab-bar status script

Optionally paint a tmux-style status strip on the far right of the desktop tab bar by running a shell command on an interval:
...
Herdr shows the first line of stdout only (Unicode is fine; ANSI styling is not applied in v1). The strip is display-only and follows desktop tab-bar visibility, including `ui.hide_tab_bar_when_single_tab`. Empty or omitted `command` disables it. Search `ui.tab_status` in the [Config reference](/docs/config-reference/) for the defaults.

The desktop tab bar also paints the active space name further right of the status strip (same label as the spaces list). It updates when you switch spaces and is hidden with the tab bar.
