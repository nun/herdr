...
`previous_attention_agent` / `next_attention_agent` cycle only through agents that are blocked or done, skipping working/idle agents. They are unset by default.
